...
#define Kb 1.3806226e-23f
#define KboQ 8.617087e-5f
#define EPS0 8.85418e-12f
...
SPICE_REAL Tb, Tc, EVjmovNv;
       ...
       Tc = XExpBV;
       Tb = 1.0f + Ijth / Isb - Tc;
       EVjmovNv = 0.5f * (Tb + SPICE_SQRT(Tb * Tb + 4.0f * Tc));
       *Vjm = Nvtm * SPICE_LOG(EVjmovNv);
...
SPICE_REAL T0, T1, T2, T3, T4, T5, T6, T7, T8, T9, Lnew=0.0f, Wnew;
...
int Size_Not_Found, i;
    ...
    {    Temp = ckt->CKTtemp;
         if (model->BSIM4v5SbulkJctPotential < 0.1f)
         {   model->BSIM4v5SbulkJctPotential = 0.1f;
             fprintf(stderr, "Given pbs is less than 0.1f. Pbs is set to 0.1f.\n");
         }
         if (model->BSIM4v5SsidewallJctPotential < 0.1f)
         {   model->BSIM4v5SsidewallJctPotential = 0.1f;
             fprintf(stderr, "Given pbsws is less than 0.1f. Pbsws is set to 0.1f.\n");
         }
         if (model->BSIM4v5SGatesidewallJctPotential < 0.1f)
         {   model->BSIM4v5SGatesidewallJctPotential = 0.1f;
             fprintf(stderr, "Given pbswgs is less than 0.1f. Pbswgs is set to 0.1f.\n");
         }

         if (model->BSIM4v5DbulkJctPotential < 0.1f)
         {   model->BSIM4v5DbulkJctPotential = 0.1f;
             fprintf(stderr, "Given pbd is less than 0.1f. Pbd is set to 0.1f.\n");
         }
         if (model->BSIM4v5DsidewallJctPotential < 0.1f)
         {   model->BSIM4v5DsidewallJctPotential = 0.1f;
             fprintf(stderr, "Given pbswd is less than 0.1f. Pbswd is set to 0.1f.\n");
         }
         if (model->BSIM4v5DGatesidewallJctPotential < 0.1f)
         {   model->BSIM4v5DGatesidewallJctPotential = 0.1f;
             fprintf(stderr, "Given pbswgd is less than 0.1f. Pbswgd is set to 0.1f.\n");
         }
         ...
         {   if (model->BSIM4v5dlcGiven && (model->BSIM4v5dlc > 0.0f))
                 model->BSIM4v5cgdo = model->BSIM4v5dlc * model->BSIM4v5coxe
                                  - model->BSIM4v5cgdl ;
             else
                 model->BSIM4v5cgdo = 0.6f * model->BSIM4v5xj * model->BSIM4v5coxe;
         }
         ...
         {   if (model->BSIM4v5dlcGiven && (model->BSIM4v5dlc > 0.0f))
                 ...
                 model->BSIM4v5cgso = 0.6f * model->BSIM4v5xj * model->BSIM4v5coxe;
         }
         if (!model->BSIM4v5cgboGiven)
             model->BSIM4v5cgbo = 2.0f * model->BSIM4v5dwc * model->BSIM4v5coxe;
         ...
         model->BSIM4v5vcrit = CONSTvt0 * SPICE_LOG(CONSTvt0 / (CONSTroot2 * 1.0e-14f));
         model->BSIM4v5factor1 = SPICE_SQRT(EPSSI / (model->BSIM4v5epsrox * EPS0)
                             * model->BSIM4v5toxe);

         Vtm0 = model->BSIM4v5vtm0 = KboQ * Tnom;
         Eg0 = 1.16f - 7.02e-4f * Tnom * Tnom / (Tnom + 1108.0f);
         ni = 1.45e10f * (Tnom / 300.15f) * SPICE_SQRT(Tnom / 300.15f)
            * SPICE_EXP(21.5565981f - Eg0 / (2.0f * Vtm0));

         model->BSIM4v5vtm = KboQ * Temp;
         Eg = 1.16f - 7.02e-4f * Temp * Temp / (Temp + 1108.0f);
         if (Temp != Tnom)
         {   T0 = Eg0 / Vtm0 - Eg / model->BSIM4v5vtm;
             T1 = SPICE_LOG(Temp / Tnom);
             T2 = T0 + model->BSIM4v5SjctTempExponent * T1;
             T3 = SPICE_EXP(T2 / model->BSIM4v5SjctEmissionCoeff);
             ...
             T3 = SPICE_EXP(T2 / model->BSIM4v5DjctEmissionCoeff);
         ...
         if (model->BSIM4v5SjctTempSatCurDensity < 0.0f)
             model->BSIM4v5SjctTempSatCurDensity = 0.0f;
         if (model->BSIM4v5SjctSidewallTempSatCurDensity < 0.0f)
             model->BSIM4v5SjctSidewallTempSatCurDensity = 0.0f;
         if (model->BSIM4v5SjctGateSidewallTempSatCurDensity < 0.0f)
             model->BSIM4v5SjctGateSidewallTempSatCurDensity = 0.0f;
         if (model->BSIM4v5DjctTempSatCurDensity < 0.0f)
             model->BSIM4v5DjctTempSatCurDensity = 0.0f;
         if (model->BSIM4v5DjctSidewallTempSatCurDensity < 0.0f)
             model->BSIM4v5DjctSidewallTempSatCurDensity = 0.0f;
         if (model->BSIM4v5DjctGateSidewallTempSatCurDensity < 0.0f)
             model->BSIM4v5DjctGateSidewallTempSatCurDensity = 0.0f;
         ...
         if (T0 >= -1.0f)
         {   model->BSIM4v5SunitAreaTempJctCap = model->BSIM4v5SunitAreaJctCap *(1.0f + T0); /*bug_fix -JX */
             model->BSIM4v5DunitAreaTempJctCap = model->BSIM4v5DunitAreaJctCap *(1.0f + T0);
         }
         else
         {   if (model->BSIM4v5SunitAreaJctCap > 0.0f)
             {   model->BSIM4v5SunitAreaTempJctCap = 0.0f;
                 fprintf(stderr, "Temperature effect has caused cjs to be negative. Cjs is clamped to zero.\n");
             }
             if (model->BSIM4v5DunitAreaJctCap > 0.0f)
             {   model->BSIM4v5DunitAreaTempJctCap = 0.0f;
                 fprintf(stderr, "Temperature effect has caused cjd to be negative. Cjd is clamped to zero.\n");
             }
         }
         T0 = model->BSIM4v5tcjsw * delTemp;
         if (T0 >= -1.0f)
         {   model->BSIM4v5SunitLengthSidewallTempJctCap = model->BSIM4v5SunitLengthSidewallJctCap *(1.0f + T0);
             model->BSIM4v5DunitLengthSidewallTempJctCap = model->BSIM4v5DunitLengthSidewallJctCap *(1.0f + T0);
         }
         else
         {   if (model->BSIM4v5SunitLengthSidewallJctCap > 0.0f)
             {   model->BSIM4v5SunitLengthSidewallTempJctCap = 0.0f;
                 fprintf(stderr, "Temperature effect has caused cjsws to be negative. Cjsws is clamped to zero.\n");
             }
             if (model->BSIM4v5DunitLengthSidewallJctCap > 0.0f)
             {   model->BSIM4v5DunitLengthSidewallTempJctCap = 0.0f;
                 fprintf(stderr, "Temperature effect has caused cjswd to be negative. Cjswd is clamped to zero.\n");
             }
         }
         T0 = model->BSIM4v5tcjswg * delTemp;
         if (T0 >= -1.0f)
         {   model->BSIM4v5SunitLengthGateSidewallTempJctCap = model->BSIM4v5SunitLengthGateSidewallJctCap *(1.0f + T0);
             model->BSIM4v5DunitLengthGateSidewallTempJctCap = model->BSIM4v5DunitLengthGateSidewallJctCap *(1.0f + T0);
         }
         else
         {   if (model->BSIM4v5SunitLengthGateSidewallJctCap > 0.0f)
             {   model->BSIM4v5SunitLengthGateSidewallTempJctCap = 0.0f;
                 fprintf(stderr, "Temperature effect has caused cjswgs to be negative. Cjswgs is clamped to zero.\n");
             }
             if (model->BSIM4v5DunitLengthGateSidewallJctCap > 0.0f)
             {   model->BSIM4v5DunitLengthGateSidewallTempJctCap = 0.0f;
         ...
         if (model->BSIM4v5PhiBS < 0.01f)
         {   model->BSIM4v5PhiBS = 0.01f;
             fprintf(stderr, "Temperature effect has caused pbs to be less than 0.01f. Pbs is clamped to 0.01f.\n");
         }
         model->BSIM4v5PhiBD = model->BSIM4v5DbulkJctPotential
                           - model->BSIM4v5tpb * delTemp;
         if (model->BSIM4v5PhiBD < 0.01f)
         {   model->BSIM4v5PhiBD = 0.01f;
             fprintf(stderr, "Temperature effect has caused pbd to be less than 0.01f. Pbd is clamped to 0.01f.\n");
         }

         model->BSIM4v5PhiBSWS = model->BSIM4v5SsidewallJctPotential
                             - model->BSIM4v5tpbsw * delTemp;
         if (model->BSIM4v5PhiBSWS <= 0.01f)
         {   model->BSIM4v5PhiBSWS = 0.01f;
             fprintf(stderr, "Temperature effect has caused pbsws to be less than 0.01f. Pbsws is clamped to 0.01f.\n");
         }
         model->BSIM4v5PhiBSWD = model->BSIM4v5DsidewallJctPotential
                             - model->BSIM4v5tpbsw * delTemp;
         if (model->BSIM4v5PhiBSWD <= 0.01f)
         {   model->BSIM4v5PhiBSWD = 0.01f;
             fprintf(stderr, "Temperature effect has caused pbswd to be less than 0.01f. Pbswd is clamped to 0.01f.\n");
         }

         model->BSIM4v5PhiBSWGS = model->BSIM4v5SGatesidewallJctPotential
                              - model->BSIM4v5tpbswg * delTemp;
         if (model->BSIM4v5PhiBSWGS <= 0.01f)
         {   model->BSIM4v5PhiBSWGS = 0.01f;
             fprintf(stderr, "Temperature effect has caused pbswgs to be less than 0.01f. Pbswgs is clamped to 0.01f.\n");
         }
         model->BSIM4v5PhiBSWGD = model->BSIM4v5DGatesidewallJctPotential
                              - model->BSIM4v5tpbswg * delTemp;
         if (model->BSIM4v5PhiBSWGD <= 0.01f)
         {   model->BSIM4v5PhiBSWGD = 0.01f;
             fprintf(stderr, "Temperature effect has caused pbswgd to be less than 0.01f. Pbswgd is clamped to 0.01f.\n");
         } /* End of junction capacitance */


         if (model->BSIM4v5ijthdfwd <= 0.0f)
         {   model->BSIM4v5ijthdfwd = 0.1f;
             fprintf(stderr, "Ijthdfwd reset to %g.\n", model->BSIM4v5ijthdfwd);
         }
         if (model->BSIM4v5ijthsfwd <= 0.0f)
         {   model->BSIM4v5ijthsfwd = 0.1f;
             fprintf(stderr, "Ijthsfwd reset to %g.\n", model->BSIM4v5ijthsfwd);
         }
         if (model->BSIM4v5ijthdrev <= 0.0f)
         {   model->BSIM4v5ijthdrev = 0.1f;
             fprintf(stderr, "Ijthdrev reset to %g.\n", model->BSIM4v5ijthdrev);
         }
         if (model->BSIM4v5ijthsrev <= 0.0f)
         {   model->BSIM4v5ijthsrev = 0.1f;
             fprintf(stderr, "Ijthsrev reset to %g.\n", model->BSIM4v5ijthsrev);
         }

         if ((model->BSIM4v5xjbvd <= 0.0f) && (model->BSIM4v5dioMod == 2))
         {   model->BSIM4v5xjbvd = 1.0f;
             fprintf(stderr, "Xjbvd reset to %g.\n", model->BSIM4v5xjbvd);
         }
         else if ((model->BSIM4v5xjbvd < 0.0f) && (model->BSIM4v5dioMod == 0))
         {   model->BSIM4v5xjbvd = 1.0f;
             fprintf(stderr, "Xjbvd reset to %g.\n", model->BSIM4v5xjbvd);
         }

         if (model->BSIM4v5bvd <= 0.0f)
         {   model->BSIM4v5bvd = 10.0f;
             fprintf(stderr, "BVD reset to %g.\n", model->BSIM4v5bvd);
         }

         if ((model->BSIM4v5xjbvs <= 0.0f) && (model->BSIM4v5dioMod == 2))
         {   model->BSIM4v5xjbvs = 1.0f;
             fprintf(stderr, "Xjbvs reset to %g.\n", model->BSIM4v5xjbvs);
         }
         else if ((model->BSIM4v5xjbvs < 0.0f) && (model->BSIM4v5dioMod == 0))
         {   model->BSIM4v5xjbvs = 1.0f;
             fprintf(stderr, "Xjbvs reset to %g.\n", model->BSIM4v5xjbvs);
         }

         if (model->BSIM4v5bvs <= 0.0f)
         {   model->BSIM4v5bvs = 10.0f;
         ...
         for (here = BSIM4v5instances(model); here != NULL;
              here = BSIM4v5nextInstance(here))
            {
              ...
              {   pParam = TMALLOC(struct bsim4v5SizeDependParam, 1);
                  ...
                  T0 = SPICE_POW(Lnew, model->BSIM4v5Lln);
                  T1 = SPICE_POW(Wnew, model->BSIM4v5Lwn);
                  ...
                  T2 = SPICE_POW(Lnew, model->BSIM4v5Wln);
                  T3 = SPICE_POW(Wnew, model->BSIM4v5Wwn);
                  ...
                  pParam->BSIM4v5leff = Lnew - 2.0f * pParam->BSIM4v5dl;
                  if (pParam->BSIM4v5leff <= 0.0f)
                  {
                  ...
                  pParam->BSIM4v5weff = Wnew - 2.0f * pParam->BSIM4v5dw;
                  if (pParam->BSIM4v5weff <= 0.0f)
                  {
                      SPfrontEnd->IFerrorf (ERR_FATAL,
                  ...
                  pParam->BSIM4v5leffCV = Lnew - 2.0f * pParam->BSIM4v5dlc;
                  if (pParam->BSIM4v5leffCV <= 0.0f)
                  {
                      SPfrontEnd->IFerrorf (ERR_FATAL,
                  ...
                  pParam->BSIM4v5weffCV = Wnew - 2.0f * pParam->BSIM4v5dwc;
                  if (pParam->BSIM4v5weffCV <= 0.0f)
                  {
                      SPfrontEnd->IFerrorf (ERR_FATAL,
                  ...
                  pParam->BSIM4v5weffCJ = Wnew - 2.0f * pParam->BSIM4v5dwj;
                  if (pParam->BSIM4v5weffCJ <= 0.0f)
                  {
                      SPfrontEnd->IFerrorf (ERR_FATAL,
                  ...
                  {   Inv_L = 1.0e-6f / pParam->BSIM4v5leff;
                      Inv_W = 1.0e-6f / pParam->BSIM4v5weff;
                      Inv_LW = 1.0e-12f / (pParam->BSIM4v5leff
                             * pParam->BSIM4v5weff);
                  }
                  else
                  {   Inv_L = 1.0f / pParam->BSIM4v5leff;
                      Inv_W = 1.0f / pParam->BSIM4v5weff;
                      Inv_LW = 1.0f / (pParam->BSIM4v5leff
                             * pParam->BSIM4v5weff);
                  }
                  ...
                  pParam->BSIM4v5abulkCVfactor = 1.0f + SPICE_POW((pParam->BSIM4v5clc
                                             / pParam->BSIM4v5leffCV),
                                             pParam->BSIM4v5cle);

                  T0 = (TRatio - 1.0f);

                  PowWeffWr = SPICE_POW(pParam->BSIM4v5weffCJ * 1.0e6f, pParam->BSIM4v5wr) * here->BSIM4v5nf;
                  ...
                  if(model->BSIM4v5tempMod == 0) {
                     ...
                     if(model->BSIM4v5rdsMod) {
                          ...
                          pParam->BSIM4v5ua = pParam->BSIM4v5ua * (1.0f + pParam->BSIM4v5ua1 * delTemp) ;
                          pParam->BSIM4v5ub = pParam->BSIM4v5ub * (1.0f + pParam->BSIM4v5ub1 * delTemp);
                          pParam->BSIM4v5uc = pParam->BSIM4v5uc * (1.0f + pParam->BSIM4v5uc1 * delTemp);
                          pParam->BSIM4v5ud = pParam->BSIM4v5ud * (1.0f + pParam->BSIM4v5ud1 * delTemp);
                          pParam->BSIM4v5vsattemp = pParam->BSIM4v5vsat * (1.0f - pParam->BSIM4v5at * delTemp);
                          T10 = 1.0f + pParam->BSIM4v5prt * delTemp;
                     if(model->BSIM4v5rdsMod) {
                          /* External Rd(V) */
                          T1 = pParam->BSIM4v5rdw * T10;
                          T2 = model->BSIM4v5rdwmin * T10;
                  ...
                  if (pParam->BSIM4v5u0 > 1.0f)
                      pParam->BSIM4v5u0 = pParam->BSIM4v5u0 / 1.0e4f;

                  /* mobility channel length dependence */
                  T5 = 1.0f - pParam->BSIM4v5up * SPICE_EXP( - pParam->BSIM4v5leff / pParam->BSIM4v5lp);
                  pParam->BSIM4v5u0temp = pParam->BSIM4v5u0 * T5
                                      * SPICE_POW(TRatio, pParam->BSIM4v5ute);
                  if (pParam->BSIM4v5eu < 0.0f)
                  {   pParam->BSIM4v5eu = 0.0f;
                      printf("Warning: eu has been negative; reset to 0.0f.\n");
                  }

                  pParam->BSIM4v5vfbsdoff = pParam->BSIM4v5vfbsdoff * (1.0f + pParam->BSIM4v5tvfbsdoff * delTemp);
                  pParam->BSIM4v5voff = pParam->BSIM4v5voff * (1.0f + pParam->BSIM4v5tvoff * delTemp);

                /* Source End Velocity Limit  */
                        if((model->BSIM4v5vtlGiven) && (model->BSIM4v5vtl > 0.0f) )
                      {
                     if(model->BSIM4v5lc < 0.0f) pParam->BSIM4v5lc = 0.0f;
                     else   pParam->BSIM4v5lc = model->BSIM4v5lc ;
                     T0 = pParam->BSIM4v5leff / (pParam->BSIM4v5xn * pParam->BSIM4v5leff + pParam->BSIM4v5lc);
                     pParam->BSIM4v5tfactor = (1.0f - T0) / (1.0f + T0 );
                       }
                  ...
                  {   T0 = pParam->BSIM4v5gamma1 * model->BSIM4v5coxe;
                      pParam->BSIM4v5ndep = 3.01248e22f * T0 * T0;
                  }

                  pParam->BSIM4v5phi = Vtm0 * SPICE_LOG(pParam->BSIM4v5ndep / ni)
                                   + pParam->BSIM4v5phin + 0.4f;

                  pParam->BSIM4v5sqrtPhi = SPICE_SQRT(pParam->BSIM4v5phi);
                  pParam->BSIM4v5phis3 = pParam->BSIM4v5sqrtPhi * pParam->BSIM4v5phi;

                  pParam->BSIM4v5Xdep0 = SPICE_SQRT(2.0f * EPSSI / (Charge_q
                                     * pParam->BSIM4v5ndep * 1.0e6f))
                                     * pParam->BSIM4v5sqrtPhi;
                  pParam->BSIM4v5sqrtXdep0 = SPICE_SQRT(pParam->BSIM4v5Xdep0);
                  pParam->BSIM4v5litl = SPICE_SQRT(3.0f * pParam->BSIM4v5xj
                                    * model->BSIM4v5toxe);
                  pParam->BSIM4v5vbi = Vtm0 * SPICE_LOG(pParam->BSIM4v5nsd
                                   * pParam->BSIM4v5ndep / (ni * ni));

                  if (pParam->BSIM4v5ngate > 0.0f)
                  {   pParam->BSIM4v5vfbsd = Vtm0 * SPICE_LOG(pParam->BSIM4v5ngate
                                         / pParam->BSIM4v5nsd);
                  }
                  else
                      pParam->BSIM4v5vfbsd = 0.0f;

                  pParam->BSIM4v5cdep0 = SPICE_SQRT(Charge_q * EPSSI
                                     * pParam->BSIM4v5ndep * 1.0e6f / 2.0f
                                     / pParam->BSIM4v5phi);

                  pParam->BSIM4v5ToxRatio = SPICE_EXP(pParam->BSIM4v5ntox
                                        * SPICE_LOG(model->BSIM4v5toxref / model->BSIM4v5toxe))
                                        / model->BSIM4v5toxe / model->BSIM4v5toxe;
                  pParam->BSIM4v5ToxRatioEdge = SPICE_EXP(pParam->BSIM4v5ntox
                                            * SPICE_LOG(model->BSIM4v5toxref
                                            / (model->BSIM4v5toxe * pParam->BSIM4v5poxedge)))
                                            / model->BSIM4v5toxe / model->BSIM4v5toxe
                                            / pParam->BSIM4v5poxedge / pParam->BSIM4v5poxedge;
                  pParam->BSIM4v5Aechvb = (model->BSIM4v5type == NMOS) ? 4.97232e-7f : 3.42537e-7f;
                  pParam->BSIM4v5Bechvb = (model->BSIM4v5type == NMOS) ? 7.45669e11f : 1.16645e12f;
                  ...
                  pParam->BSIM4v5mstar = 0.5f + SPICE_ATAN(pParam->BSIM4v5minv) / PI;
                  pParam->BSIM4v5voffcbn =  pParam->BSIM4v5voff + model->BSIM4v5voffl / pParam->BSIM4v5leff;

                  pParam->BSIM4v5ldeb = SPICE_SQRT(EPSSI * Vtm0 / (Charge_q
                                    * pParam->BSIM4v5ndep * 1.0e6f)) / 3.0f;
                  pParam->BSIM4v5acde *= SPICE_POW((pParam->BSIM4v5ndep / 2.0e16f), -0.25f);
                  ...
                  {   if (!model->BSIM4v5k1Given)
                      {
                          ...
                          pParam->BSIM4v5k1 = 0.53f;
                      }
                      if (!model->BSIM4v5k2Given)
                      {
                          if ((!ckt->CKTcurJob) || (ckt->CKTcurJob->JOBtype < 9)) /* don't print in sensitivity */
                              fprintf(stdout, "Warning: k2 should be specified with k1.\n");
                          pParam->BSIM4v5k2 = -0.0186f;
                      }
                      if ((!ckt->CKTcurJob) || (ckt->CKTcurJob->JOBtype < 9)) { /* don't print in sensitivity */
                  ...
                  {   if (!model->BSIM4v5vbxGiven)
                          pParam->BSIM4v5vbx = pParam->BSIM4v5phi - 7.7348e-4f
                                           * pParam->BSIM4v5ndep
                                           * pParam->BSIM4v5xt * pParam->BSIM4v5xt;
                      if (pParam->BSIM4v5vbx > 0.0f)
                          pParam->BSIM4v5vbx = -pParam->BSIM4v5vbx;
                      if (pParam->BSIM4v5vbm > 0.0f)
                          pParam->BSIM4v5vbm = -pParam->BSIM4v5vbm;

                      if (!model->BSIM4v5gamma1Given)
                          pParam->BSIM4v5gamma1 = 5.753e-12f
                                              * SPICE_SQRT(pParam->BSIM4v5ndep)
                                              / model->BSIM4v5coxe;
                      if (!model->BSIM4v5gamma2Given)
                          pParam->BSIM4v5gamma2 = 5.753e-12f
                                              * SPICE_SQRT(pParam->BSIM4v5nsub)
                                              / model->BSIM4v5coxe;

                      T0 = pParam->BSIM4v5gamma1 - pParam->BSIM4v5gamma2;
                      T1 = SPICE_SQRT(pParam->BSIM4v5phi - pParam->BSIM4v5vbx)
                         - pParam->BSIM4v5sqrtPhi;
                      T2 = SPICE_SQRT(pParam->BSIM4v5phi * (pParam->BSIM4v5phi
                         - pParam->BSIM4v5vbm)) - pParam->BSIM4v5phi;
                      pParam->BSIM4v5k2 = T0 * T1 / (2.0f * T2 + pParam->BSIM4v5vbm);
                      pParam->BSIM4v5k1 = pParam->BSIM4v5gamma2 - 2.0f
                                      * pParam->BSIM4v5k2 * SPICE_SQRT(pParam->BSIM4v5phi
                                      - pParam->BSIM4v5vbm);
                  }
                  ...
                  tmp = SPICE_SQRT(EPSSI / (model->BSIM4v5epsrox * EPS0)
                  ...
                  if (T0 < EXP_THRESHOLD)
                    {   T1 = SPICE_EXP(T0);
                      ...
                      pParam->BSIM4v5theta0vb0 = 1.0f / (MAX_EXP - 2.0f);
                  ...
                  tmp = SPICE_SQRT(pParam->BSIM4v5Xdep0);
                  ...
                  if(model->BSIM4v5tempMod == 2)
                        T3 = - pParam->BSIM4v5kt1 * (TRatio - 1.0f);

                  T5 = pParam->BSIM4v5k1ox * (T0 - 1.0f) * pParam->BSIM4v5sqrtPhi
                     + T3;
                  ...
                  /* stress effect */
                      ...
                      if (model->BSIM4v5wlod < 0.0f)
                  {   fprintf(stderr, "Warning: WLOD = %g is less than 0. 0.0f is used\n",model->BSIM4v5wlod);
                             wlod = 0.0f;
                  }
                  T0 = SPICE_POW(Lnew, model->BSIM4v5llodku0);
                  W_tmp = Wnew + wlod;
                  T1 = SPICE_POW(W_tmp, model->BSIM4v5wlodku0);
                  tmp1 = model->BSIM4v5lku0 / T0 + model->BSIM4v5wku0 / T1
                         + model->BSIM4v5pku0 / (T0 * T1);
                  pParam->BSIM4v5ku0 = 1.0f + tmp1;

                  T0 = SPICE_POW(Lnew, model->BSIM4v5llodvth);
                  T1 = SPICE_POW(W_tmp, model->BSIM4v5wlodvth);
                  tmp1 = model->BSIM4v5lkvth0 / T0 + model->BSIM4v5wkvth0 / T1
                       + model->BSIM4v5pkvth0 / (T0 * T1);
                  pParam->BSIM4v5kvth0 = 1.0f + tmp1;
                  pParam->BSIM4v5kvth0 = SPICE_SQRT(pParam->BSIM4v5kvth0*pParam->BSIM4v5kvth0 + DELTA);

                  T0 = (TRatio - 1.0f);
                  pParam->BSIM4v5ku0temp = pParam->BSIM4v5ku0 * (1.0f + model->BSIM4v5tku0 *T0) + DELTA;

                  Inv_saref = 1.0f/(model->BSIM4v5saref + 0.5f*Ldrn);
                  Inv_sbref = 1.0f/(model->BSIM4v5sbref + 0.5f*Ldrn);
              ...
              /*  stress effect */
              if( (here->BSIM4v5sa > 0.0f) && (here->BSIM4v5sb > 0.0f) &&
                        ((here->BSIM4v5nf == 1.0f) || ((here->BSIM4v5nf > 1.0f) && (here->BSIM4v5sd > 0.0f))) )
              {          Inv_sa = 0;
                        Inv_sb = 0;

                         kvsat = model->BSIM4v5kvsat;
                  if (model->BSIM4v5kvsat < -1.0f )
                  {   fprintf(stderr, "Warning: KVSAT = %g is too small; -1.0f is used.\n",model->BSIM4v5kvsat);
                             kvsat = -1.0f;
                      }
                      if (model->BSIM4v5kvsat > 1.0f)
                      {   fprintf(stderr, "Warning: KVSAT = %g is too big; 1.0f is used.\n",model->BSIM4v5kvsat);
                         kvsat = 1.0f;
                      }

                        for(i = 0; i < here->BSIM4v5nf; i++){
                           T0 = 1.0f / here->BSIM4v5nf / (here->BSIM4v5sa + 0.5f*Ldrn + i * (here->BSIM4v5sd +Ldrn));
                            T1 = 1.0f / here->BSIM4v5nf / (here->BSIM4v5sb + 0.5f*Ldrn + i * (here->BSIM4v5sd +Ldrn));
                           Inv_sa += T0;
                            Inv_sb += T1;
                  }
                  Inv_ODeff = Inv_sa + Inv_sb;
                  rho = model->BSIM4v5ku0 / pParam->BSIM4v5ku0temp * Inv_ODeff;
                  T0 = (1.0f + rho)/(1.0f + pParam->BSIM4v5rho_ref);
                  here->BSIM4v5u0temp = pParam->BSIM4v5u0temp * T0;

                  T1 = (1.0f + kvsat * rho)/(1.0f + kvsat * pParam->BSIM4v5rho_ref);
                  ...
                  dk2_lod = model->BSIM4v5stk2 / SPICE_POW(pParam->BSIM4v5kvth0, model->BSIM4v5lodk2) *
                                   OD_offset;
                  deta0_lod = model->BSIM4v5steta0 / SPICE_POW(pParam->BSIM4v5kvth0, model->BSIM4v5lodeta0) *
                                     OD_offset;
              ...
              { if( (!here->BSIM4v5scaGiven) && (!here->BSIM4v5scbGiven) && (!here->BSIM4v5sccGiven) )
                {   if((here->BSIM4v5scGiven) && (here->BSIM4v5sc > 0.0f) )
                          {   T1 = here->BSIM4v5sc + Wdrn;
                        T2 = 1.0f / model->BSIM4v5scref;
                        here->BSIM4v5sca = model->BSIM4v5scref * model->BSIM4v5scref
                                        / (here->BSIM4v5sc * T1);
                        here->BSIM4v5scb = ( (0.1f * here->BSIM4v5sc + 0.01f * model->BSIM4v5scref)
                                        * SPICE_EXP(-10.0f * here->BSIM4v5sc * T2)
                                        - (0.1f * T1 + 0.01f * model->BSIM4v5scref)
                                        * SPICE_EXP(-10.0f * T1 * T2) ) / Wdrn;
                        here->BSIM4v5scc = ( (0.05f * here->BSIM4v5sc + 0.0025f * model->BSIM4v5scref)
                                        * SPICE_EXP(-20.0f * here->BSIM4v5sc * T2)
                                        - (0.05f * T1 + 0.0025f * model->BSIM4v5scref)
                                        * SPICE_EXP(-20.0f * T1 * T2) ) / Wdrn;
                    } else {
                ...
                here->BSIM4v5k2 +=  pParam->BSIM4v5k2we * sceff;
                  T3 =  1.0f + pParam->BSIM4v5ku0we * sceff;
                if (T3 <= 0.0f)
                {
            ...
            if (here->BSIM4v5vtfbphi1 < 0.0f)
                here->BSIM4v5vtfbphi1 = 0.0f;

            here->BSIM4v5vtfbphi2 = 4.0f * T3;
            if (here->BSIM4v5vtfbphi2 < 0.0f)
                here->BSIM4v5vtfbphi2 = 0.0f;

            if (here->BSIM4v5k2 < 0.0f)
            {   T0 = 0.5f * pParam->BSIM4v5k1 / here->BSIM4v5k2;
                here->BSIM4v5vbsc = 0.9f * (pParam->BSIM4v5phi - T0 * T0);
                if (here->BSIM4v5vbsc > -3.0f)
                    here->BSIM4v5vbsc = -3.0f;
                else if (here->BSIM4v5vbsc < -30.0f)
                    here->BSIM4v5vbsc = -30.0f;
            }
            else
                here->BSIM4v5vbsc = -30.0f;
            ...
            here->BSIM4v5vfbzb = pParam->BSIM4v5vfbzbfactor
              ...
              lnl = SPICE_LOG(pParam->BSIM4v5leff * 1.0e6f);
              lnw = SPICE_LOG(pParam->BSIM4v5weff * 1.0e6f);
              lnnf = SPICE_LOG(here->BSIM4v5nf);
              ...
              if(here->BSIM4v5rbodyMod == 2)
                {
                  if (bodymode == 5)
                    {
                      rbsbx =  SPICE_EXP( SPICE_LOG(model->BSIM4v5rbsbx0) + model->BSIM4v5rbsdbxl * lnl +
                                    model->BSIM4v5rbsdbxw * lnw + model->BSIM4v5rbsdbxnf * lnnf );
                      rbsby =  SPICE_EXP( SPICE_LOG(model->BSIM4v5rbsby0) + model->BSIM4v5rbsdbyl * lnl +
                                    model->BSIM4v5rbsdbyw * lnw + model->BSIM4v5rbsdbynf * lnnf );
                      here->BSIM4v5rbsb = rbsbx * rbsby / (rbsbx + rbsby);


                      rbdbx =  SPICE_EXP( SPICE_LOG(model->BSIM4v5rbdbx0) + model->BSIM4v5rbsdbxl * lnl +
                                    model->BSIM4v5rbsdbxw * lnw + model->BSIM4v5rbsdbxnf * lnnf );
                      rbdby =  SPICE_EXP( SPICE_LOG(model->BSIM4v5rbdby0) + model->BSIM4v5rbsdbyl * lnl +
                                    model->BSIM4v5rbsdbyw * lnw + model->BSIM4v5rbsdbynf * lnnf );
                  ...
                  if ((bodymode == 3)|| (bodymode == 5))
                    {
                      here->BSIM4v5rbps = SPICE_EXP( SPICE_LOG(model->BSIM4v5rbps0) + model->BSIM4v5rbpsl * lnl +
                                             model->BSIM4v5rbpsw * lnw + model->BSIM4v5rbpsnf * lnnf );
                      here->BSIM4v5rbpd = SPICE_EXP( SPICE_LOG(model->BSIM4v5rbpd0) + model->BSIM4v5rbpdl * lnl +
                                             model->BSIM4v5rbpdw * lnw + model->BSIM4v5rbpdnf * lnnf );
                    }

                  rbpbx =  SPICE_EXP( SPICE_LOG(model->BSIM4v5rbpbx0) + model->BSIM4v5rbpbxl * lnl +
                                model->BSIM4v5rbpbxw * lnw + model->BSIM4v5rbpbxnf * lnnf );
                  rbpby =  SPICE_EXP( SPICE_LOG(model->BSIM4v5rbpby0) + model->BSIM4v5rbpbyl * lnl +
              ...
              {   if (here->BSIM4v5rbdb < 1.0e-3f)
                      here->BSIM4v5grbdb = 1.0e3f; /* in mho */
                  else
                      here->BSIM4v5grbdb = model->BSIM4v5gbmin + 1.0f / here->BSIM4v5rbdb;
                  if (here->BSIM4v5rbpb < 1.0e-3f)
                      here->BSIM4v5grbpb = 1.0e3f;
                  else
                      here->BSIM4v5grbpb = model->BSIM4v5gbmin + 1.0f / here->BSIM4v5rbpb;
                  if (here->BSIM4v5rbps < 1.0e-3f)
                      here->BSIM4v5grbps = 1.0e3f;
                  else
                      here->BSIM4v5grbps = model->BSIM4v5gbmin + 1.0f / here->BSIM4v5rbps;
                  if (here->BSIM4v5rbsb < 1.0e-3f)
                      here->BSIM4v5grbsb = 1.0e3f;
                  else
                      here->BSIM4v5grbsb = model->BSIM4v5gbmin + 1.0f / here->BSIM4v5rbsb;
                  if (here->BSIM4v5rbpd < 1.0e-3f)
                      here->BSIM4v5grbpd = 1.0e3f;
                  else
                      here->BSIM4v5grbpd = model->BSIM4v5gbmin + 1.0f / here->BSIM4v5rbpd;
              ...
              {
                      here->BSIM4v5grbdb = here->BSIM4v5grbsb = model->BSIM4v5gbmin;
                      here->BSIM4v5grbps = here->BSIM4v5grbpd = 1.0e3f;
                  if (here->BSIM4v5rbpb < 1.0e-3f)
                      here->BSIM4v5grbpb = 1.0e3f;
                  else
                      here->BSIM4v5grbpb = model->BSIM4v5gbmin + 1.0f / here->BSIM4v5rbpb;
              ...
              here->BSIM4v5grgeltd = model->BSIM4v5rshg * (here->BSIM4v5xgw
                      + pParam->BSIM4v5weffCJ / 3.0f / here->BSIM4v5ngcon) /
                      (here->BSIM4v5ngcon * here->BSIM4v5nf *
                      (Lnew - model->BSIM4v5xgl));
              if (here->BSIM4v5grgeltd > 0.0f)
                  here->BSIM4v5grgeltd = 1.0f / here->BSIM4v5grgeltd;
              else
              {   here->BSIM4v5grgeltd = 1.0e3f; /* mho */
                  if (here->BSIM4v5rgateMod != 0)
                  printf("Warning: The gate conductance reset to 1.0e3f mho.\n");
              }
              ...
              {
                 here->BSIM4v5sourceConductance = 0.0f;
                 ...
                 if (here->BSIM4v5sourceConductance > 0.0f)
                     here->BSIM4v5sourceConductance = 1.0f
                                            / here->BSIM4v5sourceConductance;
                 else
                 {
                     here->BSIM4v5sourceConductance = 1.0e3f; /* mho */
                     printf ("Warning: Source conductance reset to 1.0e3f mho.\n");
                 }
              } else
              {
                  here->BSIM4v5sourceConductance = 0.0f;
              ...
              {
                 here->BSIM4v5drainConductance = 0.0f;
                 ...
                 if (here->BSIM4v5drainConductance > 0.0f)
                     here->BSIM4v5drainConductance = 1.0f
                                           / here->BSIM4v5drainConductance;
                 else
                 {
                     here->BSIM4v5drainConductance = 1.0e3f; /* mho */
                     printf ("Warning: Drain conductance reset to 1.0e3f mho.\n");
                  }
              } else
              {
                  here->BSIM4v5drainConductance = 0.0f;
              ...
              {   switch(model->BSIM4v5dioMod)
                  {   case 0:
                          if ((model->BSIM4v5bvs / Nvtms) > EXP_THRESHOLD)
                              ...
                              here->BSIM4v5XExpBVS = model->BSIM4v5xjbvs * SPICE_EXP(-model->BSIM4v5bvs / Nvtms);
                          break;
                      case 1:
                          BSIM4v5DioIjthVjmEval(Nvtms, model->BSIM4v5ijthsfwd, SourceSatCurrent,
                                              0.0f, &(here->BSIM4v5vjsmFwd));
                          here->BSIM4v5IVjsmFwd = SourceSatCurrent * SPICE_EXP(here->BSIM4v5vjsmFwd / Nvtms);
                          break;
                      case 2:
                          if ((model->BSIM4v5bvs / Nvtms) > EXP_THRESHOLD)
                          {   here->BSIM4v5XExpBVS = model->BSIM4v5xjbvs * MIN_EXP;
                          ...
                          {   here->BSIM4v5XExpBVS = SPICE_EXP(-model->BSIM4v5bvs / Nvtms);
                          ...
                          here->BSIM4v5vjsmRev = -model->BSIM4v5bvs
                                             - Nvtms * SPICE_LOG((T2 - 1.0f) / model->BSIM4v5xjbvs);
                          T1 = model->BSIM4v5xjbvs * SPICE_EXP(-(model->BSIM4v5bvs
                             + here->BSIM4v5vjsmRev) / Nvtms);
                          here->BSIM4v5IVjsmRev = SourceSatCurrent * (1.0f + T1);
              ...
              {   switch(model->BSIM4v5dioMod)
                  {   case 0:
                          ...
                          here->BSIM4v5XExpBVD = model->BSIM4v5xjbvd * SPICE_EXP(-model->BSIM4v5bvd / Nvtmd);
                          break;
                      case 1:
                          BSIM4v5DioIjthVjmEval(Nvtmd, model->BSIM4v5ijthdfwd, DrainSatCurrent,
                                              0.0f, &(here->BSIM4v5vjdmFwd));
                          here->BSIM4v5IVjdmFwd = DrainSatCurrent * SPICE_EXP(here->BSIM4v5vjdmFwd / Nvtmd);
                          break;
                      case 2:
                          if ((model->BSIM4v5bvd / Nvtmd) > EXP_THRESHOLD)
                          {   here->BSIM4v5XExpBVD = model->BSIM4v5xjbvd * MIN_EXP;
                          ...
                          {   here->BSIM4v5XExpBVD = SPICE_EXP(-model->BSIM4v5bvd / Nvtmd);
                          ...
                          T0 = SPICE_EXP(here->BSIM4v5vjdmFwd / Nvtmd);
                          here->BSIM4v5IVjdmFwd = DrainSatCurrent * (T0 - here->BSIM4v5XExpBVD / T0
                                              + here->BSIM4v5XExpBVD - 1.0f);
                          ...
                          here->BSIM4v5vjdmRev = -model->BSIM4v5bvd
                                             - Nvtmd * SPICE_LOG((T2 - 1.0f) / model->BSIM4v5xjbvd); /* bugfix */
                          T1 = model->BSIM4v5xjbvd * SPICE_EXP(-(model->BSIM4v5bvd
                             + here->BSIM4v5vjdmRev) / Nvtmd);
                          here->BSIM4v5IVjdmRev = DrainSatCurrent * (1.0f + T1);
                ...
                T0 = (TRatio - 1.0f);
                model->BSIM4v5njtstemp = model->BSIM4v5njts * (1.0f + model->BSIM4v5tnjts * T0);
                model->BSIM4v5njtsswtemp = model->BSIM4v5njtssw * (1.0f + model->BSIM4v5tnjtssw * T0);
                model->BSIM4v5njtsswgtemp = model->BSIM4v5njtsswg * (1.0f + model->BSIM4v5tnjtsswg * T0);
              ...
              {
                  SPfrontEnd->IFerrorf (ERR_FATAL,
                      "detected during BSIM4v5.5.0f parameter checking for \n    model %s of device instance %s\n", model->BSIM4v5modName, here->BSIM4v5name);
                  return(E_BADPARM);
              }
         } /* End instance */
    }
